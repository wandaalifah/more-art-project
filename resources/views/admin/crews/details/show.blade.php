...
@section('content')
    <div class="px-80">
        ...
        <form action="{{ route('crews.details.destroy', [$crew->id, $project->pivot->id]) }}" method="post">
            @csrf
            @method('DELETE')
            <div class="lg:mx-28 bg-white-900 border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 text-center font-lora">
                <div class="flex justify-center mb-6">
                    <a href="{{ route('crews.details.index', $project->id) }}">
                ...
                <p class="text-xl">
                    {{ $crew->name }} as {{ $project->pivot->role }} from {{ $project->title }}?
                </p>
